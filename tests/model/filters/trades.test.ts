...
describe("sortTrades", () => {
  it("sorts by closeDate desc by default, tie-breaks by openDate asc", () => {
    const a = trade("X", "2026-04-01", "Expired", 1, 1, "2026-01-01");
    const b = trade("X", "2026-04-01", "Expired", 1, 1, "2026-02-01");
    const res = sortTrades([b, a], DEFAULT_SORT);
    expect(res.map((t) => t.openDate)).toEqual(["2026-01-01", "2026-02-01"]);
  });
...
describe("parseTradesQuery", () => {
  ...
  it("parses all params", () => {
    const sp = new URLSearchParams(
      "ticker=AAPL,NVDA&outcome=Expired,Assigned&from=2026-01-01&to=2026-04-23&sort=netPnL:asc",
    );
